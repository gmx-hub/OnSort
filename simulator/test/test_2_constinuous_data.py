...
from simulator.top.sort_top                  import sort_top
...
while True:
    ...
    input_done_vld = input_data_done_vld_i[i][j] if j < length[i] else 0
    input_config_mode = input_config_mode_i[i]

    output_vld, output_data, output_done_vld  = sort.run_one_cycle(input_data_vld_i      = input_vld,
    ...
    for port in range (const.SORT_PERF_OUT_PARALLEL):
        if isinstance(output_vld, (int, bool, np.bool_)):
            if(output_vld ==1):
                sort_result[i][oidx] = output_data
                oidx = oidx+1
        else:
            if(output_vld[port] ==1):
                sort_result[i][oidx] = output_data[port]
                oidx = oidx+1
    ...
    if(output_done_vld ==1):
        ...
        np_sort0 = np.sort(input_data_i[i])
        np_sort_data0 = np_sort0[::-1] if (input_config_mode_i[i]==1) else np_sort0
    
        compare_result[i] = np.array_equal(np_sort_data0,sort_result[i])	
		
        if(compare_result[i] ==0):	
            print("********Final resulet******")
            print("input_data_i:",input_data_i[i])
            print("np_sort_data:",np_sort_data0)
            print("sort_data   :",sort_result[i])
